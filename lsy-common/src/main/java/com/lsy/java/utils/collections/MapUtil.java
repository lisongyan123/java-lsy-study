 ...
 */
package com.lsy.java.utils.collections;
                ...
                if (val.size() > 1) {
                    ...
                    val.stream().forEach(part -> {
                        if (part instanceof FormFieldPart) {
                            ...
                            String k = fileKeyPrefix + getUUID() + "-" + fp.filename();
        ...
        src.forEach((key, value) -> {
            if (value != null) {
                target.merge(key, value, (oldValue, newValue) -> {
                    if (oldValue instanceof Map && oldValue instanceof Map) {
                        oldValue = merge((Map<String, Object>) oldValue, (Map<String, Object>) oldValue);
                        return oldValue;
                    }
                    return oldValue;
                });
